...
class PipelineConfig:
    ...
    def output_dir(self) -> Path:
        """Base output directory for all analyses"""
        if len(self.input_paths) == 1:
            return self.input_paths[0].parent
        return self.input_paths[0].parent / "batch_analysis"

    def _convert_for_yaml(self, obj):
        """Helper to convert Python objects for YAML serialization"""
        if isinstance(obj, Path):
            ...
            return [self._convert_for_yaml(v) for v in obj]
        elif hasattr(obj, '__dict__'):
            return {k: self._convert_for_yaml(v) for k, v in asdict(obj).items()}
        return obj
    ...
    def save_to_yaml(self, path: Path) -> None:
        ...
        config_dict = {
            ...
            'visualization': self._convert_for_yaml(self.visualization),
            'analysis_params': self._convert_for_yaml(self.analysis_params),
            'tracking_params': self._convert_for_yaml(self.tracking_params),
    ...
    @classmethod
    def _convert_from_yaml(cls, obj):
        """Helper to convert YAML data back to Python objects"""
        if isinstance(obj, dict):
            # Special handling for figure_size in VisualizationConfig
            if 'figure_size' in obj:
                obj['figure_size'] = tuple(obj['figure_size'])
            return {k: cls._convert_from_yaml(v) for k, v in obj.items()}
        elif isinstance(obj, list):
            return [cls._convert_from_yaml(v) for v in obj]
        return obj
    ...
    def load_from_yaml(cls, path: Path) -> 'PipelineConfig':
        ...
        input_paths = [Path(p) for p in config_dict['input_paths']]

        vis_dict = cls._convert_from_yaml(config_dict['visualization'])
        if 'output_dir' in vis_dict and vis_dict['output_dir']:
            vis_dict['output_dir'] = Path(vis_dict['output_dir'])
        visualization_config = VisualizationConfig(**vis_dict)

        analysis_dict = cls._convert_from_yaml(config_dict['analysis_params'])
        if 'data_dir' in analysis_dict and analysis_dict['data_dir']:
            analysis_dict['data_dir'] = Path(analysis_dict['data_dir'])
        analysis_config = AnalysisConfig(**analysis_dict)

        tracking_dict = cls._convert_from_yaml(config_dict['tracking_params'])
        tracking_config = TrackingParameters(**tracking_dict)

        return cls(
            input_paths=input_paths,
            steps_to_run=config_dict['steps_to_run'],
            visualization=visualization_config,
            analysis_params=analysis_config,
            tracking_params=tracking_config,
            batch_mode=config_dict.get('batch_mode', False)
        )
